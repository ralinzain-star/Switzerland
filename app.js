...
const I18N = {
  ...
  retro_sub:      { zh:'每日評分、景點打卡、預算回顧 — 內容只存在這台裝置', en:'Daily ratings, spot check-ins, budget review — stored on this device only', de:'Tagesbewertung, Orte, Budget — nur auf diesem Gerät gespeichert' },
  retro_banner_before: { zh:'旅程尚未開始 — 這是預覽。現在填的內容會保留，出發後可以每天回來記一句。', en:'Trip has not started — preview mode. Anything you write is kept, so you can log a line each day once you go.', de:'Reise noch nicht gestartet — Vorschau. Eingaben bleiben erhalten.' },
  retro_banner_during: { zh:'旅程進行中 — 建議每天睡前花 1 分鐘評分＋寫一句，回來整理才不會忘。', en:'Trip in progress — take a minute each night to rate the day and jot one line.', de:'Reise läuft — jeden Abend kurz bewerten und eine Zeile notieren.' },
  retro_banner_after:  { zh:'旅程已結束 — 趁記憶還新，把評分、打卡與心得補完，最後匯出留存。', en:'Trip finished — fill in the ratings, check-ins and notes while it is fresh, then export.', de:'Reise beendet — jetzt ausfüllen und exportieren.' },
  retro_stat_rating:   { zh:'平均評分', en:'Avg rating', de:'Ø Bewertung' },
  retro_stat_rating_sub:{ zh:'{n} 天已評分', en:'{n} days rated', de:'{n} Tage bewertet' },
  retro_stat_visited:  { zh:'景點打卡', en:'Spots visited', de:'Besuchte Orte' },
  retro_stat_visited_sub:{ zh:'共 {total} 個候選', en:'of {total} shortlisted', de:'von {total} geplant' },
  retro_stat_spent:    { zh:'實際花費', en:'Actual spend', de:'Ist-Ausgaben' },
  retro_stat_spent_sub:{ zh:'每人', en:'per person', de:'pro Person' },
  retro_stat_days:     { zh:'旅程天數', en:'Days', de:'Tage' },
  retro_stat_days_sub: { zh:'{a} – {b}', en:'{a} – {b}', de:'{a} – {b}' },
  retro_days_title:    { zh:'每日回顧', en:'Day by day', de:'Tag für Tag' },
  retro_note_ph:       { zh:'這天最記得的一件事…', en:'The one thing you want to remember…', de:'Woran du dich erinnern willst…' },
  retro_unrated:       { zh:'未評分', en:'Not rated', de:'Ohne Bewertung' },
  retro_budget_title:  { zh:'預算回顧（預估 vs 實際）', en:'Budget review (estimate vs actual)', de:'Budget: Plan vs. Ist' },
  retro_diff_under:    { zh:'比預估少花 {amt}', en:'{amt} under budget', de:'{amt} unter Budget' },
  retro_diff_over:     { zh:'比預估多花 {amt}', en:'{amt} over budget', de:'{amt} über Budget' },
  retro_diff_even:     { zh:'與預估打平', en:'Right on budget', de:'Genau im Budget' },
  retro_budget_hint:   { zh:'實際金額來自「花費」分頁（已購項目＋每日記帳）。上排為預估、下排為實際。', en:'Actuals come from the Expenses tab (purchased items + daily log). Top bar = estimate, bottom bar = actual.', de:'Ist-Werte aus dem Ausgaben-Tab. Oben Plan, unten Ist.' },
  retro_pois_title:    { zh:'景點打卡', en:'Spot check-in', de:'Orte abhaken' },
  retro_poi_went:      { zh:'去過', en:'Went', de:'War da' },
  retro_poi_skipped:   { zh:'沒去成', en:'Missed', de:'Verpasst' },
  retro_missed_title:  { zh:'下次再來', en:'Next time', de:'Beim nächsten Mal' },
  retro_missed_empty:  { zh:'還沒有標記「沒去成」的景點。', en:'Nothing marked as missed yet.', de:'Noch nichts als verpasst markiert.' },
  retro_notes_title:   { zh:'心得', en:'Takeaways', de:'Fazit' },
  retro_note_best:     { zh:'最推薦', en:'Best of the trip', de:'Highlight' },
  retro_note_best_ph:  { zh:'一定要跟別人說的那一個…', en:'The one you would tell a friend about…', de:'Das eine Highlight…' },
  retro_note_worst:    { zh:'最可惜／最雷', en:'Biggest letdown', de:'Grösster Reinfall' },
  retro_note_worst_ph: { zh:'不值得、太趕、或下次會跳過的…', en:'Not worth it, too rushed, would skip…', de:'Nicht lohnenswert, zu hektisch…' },
  retro_note_surprise: { zh:'意外的驚喜', en:'Nicest surprise', de:'Schönste Überraschung' },
  retro_note_surprise_ph:{ zh:'沒排進行程卻很棒的…', en:'Unplanned but great…', de:'Ungeplant, aber toll…' },
  retro_note_next:     { zh:'下次會這樣改', en:'Do differently next time', de:'Nächstes Mal anders' },
  retro_note_next_ph:  { zh:'天數、動線、訂票時機、行李…', en:'Length, routing, booking timing, packing…', de:'Dauer, Route, Buchung, Gepäck…' },
  retro_changelog_title:{ zh:'行程變更記錄', en:'Plan changes', de:'Planänderungen' },
  retro_lessons_title: { zh:'規劃心得', en:'Planning lessons', de:'Erkenntnisse' },
  retro_export_json:   { zh:'匯出 JSON', en:'Export JSON', de:'JSON exportieren' },
  retro_export_text:   { zh:'複製成文字', en:'Copy as text', de:'Als Text kopieren' },
  retro_import:        { zh:'匯入 JSON', en:'Import JSON', de:'JSON importieren' },
  retro_clear:         { zh:'清除全部', en:'Clear all', de:'Alles löschen' },
  retro_copied:        { zh:'已複製到剪貼簿', en:'Copied to clipboard', de:'In Zwischenablage kopiert' },
  retro_copy_failed:   { zh:'複製失敗，請手動選取', en:'Copy failed — select manually', de:'Kopieren fehlgeschlagen' },
  retro_imported:      { zh:'已匯入', en:'Imported', de:'Importiert' },
  retro_import_failed: { zh:'檔案格式不正確', en:'Invalid file', de:'Ungültige Datei' },
  retro_clear_confirm: { zh:'確定清除這台裝置上的所有回顧內容？此動作無法復原。', en:'Clear every retro entry stored on this device? This cannot be undone.', de:'Alle Rückblick-Einträge auf diesem Gerät löschen?' },
  retro_saved_hint:    { zh:'自動儲存在這台裝置的瀏覽器（換裝置請用匯出／匯入）', en:'Auto-saved in this browser (use export/import to move devices)', de:'Automatisch in diesem Browser gespeichert' },
